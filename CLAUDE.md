...
- Footer-социалки (Telegram/WhatsApp/Карта) ещё `href="#"` — нужны реальные
  адреса либо скрыть.
- **Аудит `extractElectricalProductType` по всей БД не запускался на проде**
  (2026-07-01). Класс бага: текстовые эвристики `product-attributes.ts` не
  знали реальную категорию товара, из-за чего слово-триггер («кабель»,
  «box», «switch», «разъём») в описании интерфейса/упаковки другого товара
  (камера, кулер ПК, ТВ, Nintendo Switch) давало ложный
  `Тип электротовара`. Точечно исправлено (гейт по слову-соседу + опциональный
  `categoryName`-параметр в `extractProductNameAttributes`/`extractProductNameSpecs`/
  `buildProductAttributeRows`, используется только для категорий
  «камеры»/«кулеры»), но гейт по категории не покрывает остальные ветки
  (Розетка/Автомат/Светильник/Терморегулятор/Рамка) и другие категории.
  Написан read-only скрипт `web-store/scripts/audit-electrical-false-positives.ts`
  (`npm run audit:electrical` из `web-store/`) — прогоняет парсер по всем
  активным товарам и печатает подозрительные комбинации (electrical-тип +
  camera/cooling-факт, cable_cores/section у верхней границы правдоподобия).
  **Нужен доступ к прод-БД, чтобы реально его запустить** — этого доступа в
  сессии, где писался фикс, не было. Следующая сессия с доступом к БД должна
  прогнать и разобрать находки.
