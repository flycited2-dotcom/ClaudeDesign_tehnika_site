...
### 2026-08-14 — B2B-выдача по 20 и монитор только по изменениям ✅

- **Commit:** `62d7e0b`. **Backup:** `/var/www/climat-simf.ru.source-backup-20260814111249.tar.gz`.
- `B2B_ASSISTANT_SEARCH_LIMIT` поднят с 5 до 20 (новый допустимый максимум тоже 20). Первые 500 совпадений ранжируются в памяти, поэтому точные SKU/артикулы сохраняют приоритет, а голый запрос `стабилизатор` поднимает стабилизаторы электрического напряжения выше селфи-палок и телефонных gimbal-устройств.
- Клиентская карточка использует общий `warrantyLabel`: значение поставщика `0` больше не показывается. Фиктивные `12 месяцев` не подставляются; реальный ненулевой срок поставщика форматируется как прежде.
- Причина production-ошибки остатков установлена измерением API: login I-T-P отвечал за ~0,27 с, но нефильтрованный `get_active_products` зависал без первого байта дольше 25–75 с. Монитор переведён на один согласованный пакетный запрос `sku IN [21 отслеживаемый SKU]`; проверка успешно вернула четыре активных товара вместо загрузки всего каталога.
- Повторная рассылка всех доступных карточек каждые 15 минут удалена. Состояние теперь сравнивает доступность, символьный и точный остаток своего/ближайшего склада и закупочную цену. Полные карточки отправляются только для реально изменившихся доступных товаров; исчезнувшие остатки также уведомляются. При полном отсутствии изменений отправляется одна компактная строка `Остатки актуальны — изменений нет` не чаще раза в 60 минут, при этом системный timer продолжает проверять API каждые 15 минут.
- В карточке монитора точный `real_qty`, когда он дан API, показывается как количество штук вместо грубой метки `мало`. Старый state v1 мигрирует без ложной единовременной рассылки: отсутствующие ранее price/realQty принимаются за неизвестный baseline и сохраняются на первом успешном запуске.
- Telegram-системную подпись `via @bot` скрыть через Bot API нельзя: inline-сообщение получает `via_bot_id`, а клиент Telegram обязан показать кликабельный username. Allow-list по Telegram ID остаётся защитой: посторонний может открыть публичный профиль бота, но inline-запрос получает пустую выдачу, а приватный поиск отклоняется. Если потребуется полностью убрать `via`, нужно отказаться от inline-mode в пользу скачиваемой/копируемой карточки или отдельной Mini App/web-share схемы.
- **Проверено:** 346/346 тестов на bundled Node 24, ESLint изменённых файлов и production build. Production-запрос `стабилизатор`: `total=472`, `shown=20`, все первые 20 результатов — электрические стабилизаторы; monitor run: `checked=21`, `available=4`, `notified=0`, `changed=0`, состояние содержит точные остатки 2/3/5/2. Сервисы `climat-simf-b2b-assistant` и timer активны, следующий запуск по расписанию, сайт отвечает HTTP 200.

### 2026-08-14 — Пагинация поиска B2B Telegram-ассистента ✅

- **Commit:** `2d045c6`. **Backup:** `/var/www/climat-simf.ru.source-backup-20260814001352.tar.gz`.
- Удалён фактический потолок в 5/10 товаров. Новый `searchB2bProductPage` считает полное число совпадений, закрепляет точные SKU/артикулы наверху и выдаёт стабильные страницы по `B2B_ASSISTANT_SEARCH_LIMIT` (production: 5) без повторения уже показанных позиций.
- В Telegram заголовок показывает `Найдено товаров: N` и диапазон `1–5`; после карточек появляется `Показать ещё 6–10`. По нажатию старая кнопка удаляется, отправляются следующие пять карточек и новая кнопка продолжения.
- Запрос хранится 60 минут в новой PostgreSQL-модели `TelegramSearchSession` под коротким кодом; callback укладывается в Telegram `callback_data` и привязан к Telegram ID владельца поиска.
- **Проверено:** 340/340 тестов, ESLint, production build и `prisma db push`; сервис `active`, `NRestarts=0`. Production-запрос `стабилизатор` нашёл 459 товаров; страницы 1–5 и 6–10 имеют разные SKU без пересечений. Первая тестовая страница с кнопкой отправлена владельцу ботом.

### 2026-08-13 — Мобильный B2B Telegram-ассистент: первый рабочий срез ✅

- **Commit:** `a076692`. **Backup:** `/var/www/climat-simf.ru.source-backup-20260813225315.tar.gz`.
- Развёрнута ветка `codex/b2b-telegram-assistant`, основанная на рабочем мониторинге ExeGate и Telegram-заказах. Добавлен отдельный long-polling сервис `climat-simf-b2b-assistant` с systemd `Restart=always`, чистым перезапуском каждые 12 часов и сохранением `getUpdates` offset.
- Новый бот использует общую PostgreSQL-базу сайта: поиск по названию, модели, бренду, штрихкоду, SKU и категории; менеджерские карточки показывают закупочную цену и остаток, клиентские карточки скрывают поставщика, закупку и внутренний SKU.
- Реализованы пресеты/ручной ввод наценки, подписанное клиентское предложение и передача карточки через Telegram inline mode. Заказ переиспользует существующую подписанную `/stock-order/<sku>` форму и по-прежнему не подтверждает отгрузку автоматически.
- Секреты нового бота записаны только в серверный `.env` (mode `600`), allow-list ограничен владельцем. Серверные `.env` backups: `.env.bak-b2b-20260813225517` и `.env.bak-b2b-20260813225547`.
- **Проверено:** локально 331/331 тест, ESLint новых файлов, production build; deploy healthcheck; Telegram `getMe` (`@b2b_asist_me_bot`, inline=true), webhook пуст; тестовое сообщение владельцу доставлено; сервис `enabled/active`, `NRestarts=0`; серверный поиск `ExeGate AVS-8000` вернул SKU `10539750` с актуальной ценой и наличием.
- **Известное ограничение:** живой пользовательский путь поиска/кнопок нужно подтвердить в Telegram после первого сообщения владельца. Наблюдение, избранное, история и наценки по категориям остаются следующими этапами.

#### Hotfix: доставка фотографий в Telegram

- **Commit:** `a900bac`. **Backup:** `/var/www/climat-simf.ru.source-backup-20260813230740.tar.gz`.
- Первый живой поиск нашёл две позиции, но Telegram отклонил первую карточку: `sendPhoto: Bad Request: failed to get HTTP URL content`. Публичный image-proxy при этом исправно отдавал `200 image/webp`; проблема возникала при самостоятельной загрузке URL серверами Telegram.
- Исправлено: B2B-процесс теперь сам скачивает фото через публичный proxy и загружает его в Telegram multipart-файлом. Любая ошибка изображения переводит карточку на текстовый fallback с сохранением всех кнопок. Inline-результаты используют надёжный article-формат и не зависят от доступности картинки.
- **Проверено:** 331/331 тест, ESLint, production build; сервис перезапущен, `active`, `NRestarts=0`; принудительный production-запрос `AVS-8000` от разрешённого Telegram ID успешно отправил найденные карточки без ошибки.

#### UX hotfix: короткий код клиентского предложения

- **Commit:** `355a531`. **Backup:** `/var/www/climat-simf.ru.source-backup-20260813234836.tar.gz`.
- Длинный подписанный `offer_<payload>.<signature>` в строке inline-режима заменён на случайный шестизначный код без неоднозначных символов, например `K7M4Q2`. SKU, клиентская цена, владелец предложения и срок действия хранятся в новой PostgreSQL-модели `TelegramClientOffer`; сам код не содержит деловых данных.
- Предложение действует 24 часа и разрешается только тому Telegram manager ID, который его создал. Просроченные записи удаляются лениво при создании/открытии. Старые подписанные `offer_...` сохранены совместимыми до окончания их срока действия.
- **Проверено:** 335/335 тестов, ESLint, production build и `prisma db push`; сервис `active`, `NRestarts=0`; на production создано и доставлено владельцу тестовое предложение `PKXWVQ` длиной 6 символов для SKU `10539750`.
...
- **Контекст:** Iter 74 оставил 3 пункта заблокированными (аудит `/admin`, реальные опт-цены b2b,
  Gmail-доставляемость). Владельцу заданы явные вопросы через `AskUserQuestion`; ответы:
  admin — **только code-review без выдачи креды**; опт-цены — **подтверждено, другого источника
  нет, наценочная модель остаётся навсегда, не долг**; Gmail — **отложить**.
- **Аудит `/admin`**: прочитаны все 17 файлов (страницы + client-формы) и все 3 файла server
  actions (`admin/actions.ts`, `admin/leads/actions.ts`, `admin/role-requests/actions.ts`) без
  живого захода в панель. Проверено систематически: authz на каждом action (все вызывают
  `requireAdmin()`), edge-гейт в `middleware.ts` (дублирующая проверка по cookie — уже была,
  корректна), exhaustiveness `orderStatusMeta`/`TYPE_LABEL`/`TYPE_BADGE` под enum'ы (типобезопасно),
  корректность SKU-поиска (`sku: Int`, `Number(q)` без проблем с ведущими нулями).
  - **Найден и исправлен реальный баг**: кнопки «Запустить» на `/admin/sync` не имели защиты от
    повторного запуска. `syncItpProducts`/`syncItpImages`/`syncItpCategories` делают
    построчный `await prisma.upsert(...)` в цикле без батчинга по всему фиду поставщика
    (~340k товаров/~4.3k категорий) — заведомо дольше 180s `proxy_read_timeout` в nginx.
    Админ, увидев таймаут в браузере, мог кликнуть «Запустить» ещё раз — сервер продолжает
    выполнять первый запуск (HTTP-обрыв не убивает уже запущенный server action), второй клик
    запускал ВТОРОЙ параллельный синк по тем же строкам (гонка записи, включая общий
    `updateMany({isActive: false})` в начале каждого синка).
  - **Фикс**: `web-store/src/lib/sync-lock.ts` — `isFreshRunningLock(startedAt, now)`, окно 3ч.
    `runSyncAction` перед запуском проверяет последний `SyncLog{type, status:"running"}` и
    пропускает запуск, если лок ещё свежий; `/admin/sync` дизейблит кнопку и меняет подпись на
    «Уже выполняется…», если для этого типа есть свежий running-лог. Staleness-окно специально
    не бесконечное: если синк убит на середине (`pm2 stop` во время `--full-clean` деплоя —
    задокументированный сценарий из Iter 74), лок не должен виснуть навечно и блокировать все
    будущие синки этого типа.
  - При рефакторинге поймал новый для проекта lint-паттерн: `react-hooks/purity`
    (React Compiler) запрещает вызов `Date.now()` прямо в теле async Server Component —
    поймано на `/admin/sync/page.tsx`. Решение — вынести вызов в обычную (не-компонентную)
    функцию `isFreshRunningLock` с `now = Date.now()` как default-параметром; компонент зовёт
    только эту функцию, не ссылается на `Date.now` напрямую.
  - Остального критичного не найдено за это чтение (settings/products/orders/leads/categories/
    role-requests/logs — все консистентны, zod-валидация и транзакции на месте).
  - **Открытый остаток**: живой клик-прогон панели глазами — владелец не даёт креды на этом
    этапе, это осталось долгом (см. `CLAUDE.md` → Известные долги).
- **Опт-цены b2b и Gmail** — закрыты как решения владельца, не как код. См. `ROADMAP.md` (§5, §10)
  и `CLAUDE.md` → Известные долги за точную формулировку.
- 303 теста (было 300), lint/build чисты.
- **Smoke на проде**: `/`, `/catalog`, `/b2b`, `/gov`, `/compare`, `/favorites`, `/cart`,
  `/admin/login` — все 200.
...
### 2026-07-01 — Iter 74: продолжение брейншторма — 6 из 9 пунктов ✅ (3 упёрлись в реальные блокеры)

- **Commits:** `6c9136c` (footer) → `53668a9` (b2b/gov КП-история) → `7fb4983` (fix: pm2 stop перед full-clean) → `d808178` (ТВ-резолюция) → `0519c06` (недавно смотрели) → `8122ea3` (уведомить о поступлении) → `f6a9ce6` (on-demand revalidate).
- **Что вошло:**
  1. **Футер** — реальный Telegram `@B2B_opt_simf` (было `t.me/+79785792995` — угаданная ссылка из прошлой сессии), WhatsApp убран полностью (у бизнеса его нет — старый `wa.me` тоже был угадан).
  2. **История заявок КП для b2b/gov** — `Lead.userId` (новое поле, миграция через `prisma db push`), `requestQuoteAction` пишет `userId` вошедшего пользователя, `getLeadsForUser()` + общий компонент `LeadHistoryList` на `/b2b` и `/gov` (зеркалит `/account` из Iter 69).
  3. **Характеристики ТВ** — резолюция теперь ловит 8K/HD Ready/голый UHD (безопасно везде) + числовые форматы (1920x1080 и т.п.) через строгий whitelist из 6 известных разрешений, гейтнутый на `looksLikeTvProduct` (не тот же класс бага, что чинили с кабелем — здесь точное совпадение по таблице, не диапазон).
  4. **«Вы недавно смотрели»** — тот же localStorage-паттерн, что избранное/сравнение (`sku-list-storage.ts`), капнуто на 12, рендерится на `/product/[slug]` в `.section-head`/`.product-grid` как «Рекомендуемые товары» на главной.
  5. **«Уведомить о поступлении»** — новый `LeadType.STOCK_ALERT`, та же durable-lead схема, что у callback/КП (попадает в `/admin/leads`, телеграм-уведомление менеджеру best-effort). НЕ автоматическая рассылка при смене остатка — как и все остальные заявки, обрабатывается человеком.
  6. **On-demand ревалидация** — `POST /api/admin/revalidate` (secret-заголовок) + 5 sync-скриптов дёргают его последним шагом. **Не активна без ручного шага на VPS** (см. ограничения ниже).
- **Найден и исправлен реальный баг деплоя**: `--full-clean` дважды подряд падал детерминированно на `rm: cannot remove '.next/cache/fetch-cache': Directory not empty` — живой pm2-процесс писал в эту директорию, пока `rm -rf .next` пытался её удалить. Починено остановкой pm2 ДО очистки при full-clean (жертвуем секундами даунтайма ради надёжности, обычный путь без full-clean не тронут).
- **Известные ограничения:**
  - `REVALIDATE_SECRET` нужно вручную добавить в `/var/www/climat-simf.ru/.env` — доступа на запись в .env на VPS в этой сессии не было (намеренно, не через SSH). До этого шага фича — безопасный no-op.
  - **Не проверено эмпирически**: реагирует ли `unstable_cache`-тег на новую сигнатуру `revalidateTag(tag, {expire:0})` в этой версии Next 16 — багажные доки говорят только про `fetch next.tags` и новый `cacheTag()`/`"use cache"`, не про `unstable_cache` явно. Риск низкий — воркер прогрева остаётся страховкой в любом случае.
  - **3 пункта не сделаны — упёрлись в блокеры, не в объём работы**: аудит `/admin` (нет ADMIN_EMAIL/ADMIN_PASSWORD в этой сессии), реальные опт-цены b2b (фид поставщика не отдаёт wholesale-цены вообще, нужен другой источник данных — бизнес-решение), доставляемость Gmail (нужен доступ к Google Postmaster Tools — отдельный Google-аккаунт). Ждут ответа владельца.
- 300 тестов (было 277 в начале сессии Iter 71), lint/build чисты на каждом шаге.

### 2026-07-01 — Iter 73: брейншторм-раунд по каталогу (11 пунктов) ✅

- **Commits:** `6e57af3` (share/sticky-compare/in-stock filter) → `afb1784` (пустые категории мегаменю + SEO JSON-LD) → `8ad624a` (generic --run-audit + BackOrder + catalog-audit-report.ts) → `00676a6` (network switch fix ч.1 + ревью-полировка) → `77b5f57` (network switch fix ч.2) · **Backups:** `…162148`, `…163130`, `…165108` (последний — финальный, чистый).
- **Контекст:** владелец утвердил весь список брейнторма из предыдущего ответа + добавил пункт «кнопка Поделиться». 11 задач, реализовано/закрыто с выводом каждая.
- **Что вошло (код):**
  1. **Кнопка «Поделиться»** на `/product/[slug]` (Web Share API + fallback на `clipboard.writeText` с временной иконкой-галочкой, `setTimeout` корректно чистится на unmount).
  2. **ROADMAP #7 sticky-колонка сравнения** — `.compare-th-label`/`.compare-row-label` теперь `position: sticky` на `/compare`, не теряют контекст характеристики при горизонтальном скролле на узких экранах.
  3. **ROADMAP #8 фильтр «В наличии»** — оказалось, `Product.stockStatus` (out/low/available/plenty из фида поставщика) уже был в схеме и отображался (`StockBadge`), просто не использовался как фильтр (`available` фильтрует по другому полю — `isAvailable`, «доступно к заказу»). Добавлен `onlyInStock` сквозь всю цепочку (`catalog-query.ts` → `catalog.ts` WHERE `stockStatus != "out"` → чекбокс в существующем аккордеоне «Наличие» → pill → noindex для отфильтрованных URL).
  4. **Пустые категории в мегаменю** — `/api/catalog/categories` возвращал leaf-категории (`hasChildren=false`) с `productCount=0`; они были кликабельны и вели на реально пустую `/catalog/[slug]`. Подтверждено вживую («Аудиоредакторы», «Графические и видеоредакторы»). Пофикшено — leaf с 0 товаров теперь не возвращается (non-leaf с 0 не трогаем, т.к. это без-роллап-счётчик и там могут быть непустые потомки).
  5. **SEO JSON-LD**: `stockStatus` → точный `schema.org` availability (`LimitedAvailability`/`InStock`/`OutOfStock`/**`BackOrder`** — последний именно когда физически нет на складе, но товар всё ещё можно заказать «под заказ 7 дней», это заметили при живой проверке и добавили отдельным фиксом); `mpn` из партномера; `BreadcrumbList` теперь и на страницах категорий (раньше — только на карточке товара).
  6. **Аудит остальных веток парсера** (`scripts/catalog-audit-report.ts`, `npm run audit:catalog`) — по образцу `audit-electrical`: cross-family коллизии (типовой ключ одного семейства + типовой ключ другого на одном товаре), покрытие характеристик по 8 не аудированным семействам, подсчёт `name IS NULL`. Прогнан на 341 216 товарах 3 раза (430→7→**1** после двух раундов фиксов): найден и закрыт баг — сетевые коммутаторы (TFortis/Fanvil/ORIGO/SNR) ложно получали `electrical_product_type` через **4 разных триггера**: английское «Switch» (→ Выключатель), «разъем для АКБ» (→ Коннектор), модельный суффикс «-Box» у TFortis (→ Коробка/щит), «2-проводной» у Fanvil (→ Кабель). Исправлено одним общим гейтом `looksLikeNetworkProduct(text)`, вынесенным в `isNetworkProduct` в начале `extractElectricalProductType`. Финальный прогон — 1 находка, и та benign true positive (комбо духовка+варочная поверхность «Жарочный шкаф+2конф.»).
- **Investigation-задачи, закрытые выводом без кода** (см. task metadata в сессии, кратко):
  - **Расширение характеристик за 5 категорий Iter 70**: данные покрытия собраны (TV 18.4% при 2079 кандидатах — лучший следующий кандидат; oven 39.5%, dishwasher 43.4% — вторичные; tire 0.0% — похоже на артефакт грубого regex-подсчёта, не реальный баг). Не реализовано — каждое семейство размером с отдельную итерацию (по прецеденту Iter 70), не разовый фикс.
  - **Структурированные атрибуты поставщика для расходников**: фид ITP (`ItpProduct` тип) в принципе не содержит характеристик ни в каком поле — только name/vendor/part/warranty/weight/volume. `Product.specifications Json?` в схеме — мёртвое неиспользуемое поле. `name`-парсинг — единственный практический путь.
  - **Читаемые названия для `name=null`**: оказалось, `name IS NULL` — **100% каталога** (341216/341216), не «часть товаров». `name` — ручное admin-поле, которое никогда не использовалось (ожидаемо для 341k SKU). `supplierName` (универсальный fallback) уже достаточно читаем судя по десяткам реальных примеров, увиденных за сессию. Реального пробела для фикса нет.
  - **Cold-start через periodic warmer вместо ISR**: корневая причина — bulk-синк скрипты (`sync-products.ts` и т.п.) работают как отдельные Node-процессы вне Next.js-сервера (cron/ручной `tsx`), поэтому структурно не могут звать `revalidateTag`/`revalidatePath` (те существуют только в контексте работающего Next-процесса). Решение — защищённый `/api/admin/revalidate`, который синк-скрипты дёргают curl'ом последним шагом. Спроектировано, не реализовано (отдельный проект среднего размера).
- **Ловушка деплоя, найденная и закрытая в процессе**: `--sync-attributes`/`--run-audit` в синхронном режиме (не в фоне) дважды подряд обрывали SSH-сессию без единого байта вывода прямо на старте долгой тихой DB-операции (bulk DELETE по ~1-2 млн строк) — похоже на обрыв по таймауту простоя NAT/сети. Починено `transport.set_keepalive(30)` в `deploy_vps.py` + периодические прогресс-логи в `sync-product-attributes.ts`. Заодно кодировка вывода (`sys.stdout.reconfigure(encoding="utf-8")`) — кириллица в первом успешном аудите была нечитаемым мусором из-за консольной кодировки Windows.
- **Независимая проверка**: `superpowers:code-reviewer` агент прогнан на диффе первых 3 коммитов — критичных багов не нашёл, 4 мелких замечания (2 исправлены: несовпадение прозрачности sticky-колонки, незачищенный `setTimeout`; 2 информационные, без действия).
- **Проверено на живых SKU**: коммутатор Fanvil PN24 (SKU 10946369) — было бы `electrical_product_type=Кабель`, стало только корректное `Тип управления: Неуправляемый`; мегаменю → «Программное обеспечение» больше не показывает 2 пустые подкатегории; фильтр `?stock=1` и чекбокс «В наличии на складе» — на месте.
- **Итог**: 293→**297** тестов (vitest, было в начале сессии — 277 до старта Iter 71), 10 python-тестов (было 6), lint/build чисты на каждом шаге. `deploy_vps.py` дополнен: `--full-clean`, `--sync-attributes`, `--run-audit <npm-script>` (обобщён), keepalive, UTF-8 stdout.
- **Известные ограничения**: `stockStatus`-фильтр не выставлен ни в одной готовой `podborki`-пресете (только на `/catalog`, `/catalog/[slug]`, `/search`) — не баг, просто ни один пресет пока не просит его. Investigation-выводы выше (TV-характеристики, revalidate-webhook) — кандидаты на будущие итерации, не долги в смысле «сломано сейчас».

### 2026-07-01 — Iter 72: гейт electrical_product_type на весь каталог + аудит закрыт (0 ложных) ✅

- **Commits:** `eaf26f4` (feat: catalog-wide category gate), `b068594` (fix: SSH keepalive + progress log), `34e6b4e` (fix: cable-канал/стяжка/наконечник) · **Backups:** `…135403.tar.gz` (первая попытка ресинка, упала на SSH-таймауте — см. ниже) → `…142309.tar.gz` (успешный ресинк+аудит, 430 находок) → `…144140.tar.gz` (финальный, 65 находок — все настоящие кабели).
- **Контекст:** продолжение Iter 71 по запросу владельца — довести до конца пункты «прогнать аудит по всей БД» и «сделать категорийный гейт на весь каталог, а не только камеры/кулеры».
- **Что вошло:**
  - `isKnownNonElectricalCategory` заменён на allowlist `looksLikeElectricalAccessoryCategory` — стемы реальных категорий каталога (кабель, провод, розетка, выключатель, автомат, светильник, разъём, клемма, терморегулятор, рамка, короб/щит, заземление, рубильник, контактор, удлинитель, патрон и т.д., ~184 из 1785 непустых категорий сайта). Теперь ЛЮБАЯ известная категория вне этого домена (не только камеры/кулеры — ноутбуки, ТВ, шины, что угодно) гасит generic electrical-классификацию, даже для голых модельных кодов без текстовых подсказок. Категория неизвестна/не задана → как раньше, только текстовые эвристики.
  - Запущен `npm run audit:electrical` по всему каталогу (341 216 товаров) — 1-й прогон нашёл 430 подозрительных фактов, ВСЕ одного паттерна: «кабель-канал» (короба/лотки для проводки), «стяжка кабельная» (хомуты) и «наконечник кабельный» — их размер W×H/W×L («40х20», «4.8х300» и т.п.) читался как жилы×сечение, хотя сам товар — аксессуар для прокладки кабеля, а не кабель. Добавлен `looksLikeCableAccessoryProduct` (тот же паттерн guard, что и interface-port-spec/packaging-box/nintendo-switch). 2-й прогон аудита — **65 находок, все настоящие силовые кабели 240мм² (легитимное сечение для промышленных кабелей)** — ложных срабатываний по всему каталогу больше нет.
  - `product-attribute-backfill.ts`/`sync-product-attributes.ts` пересинканы дважды (после генерализации гейта и после фикса кабель-аксессуаров) — все persisted `ProductAttribute` (source="name") в БД теперь соответствуют исправленному коду.
  - 291 тест (было 286), lint/build чисты.
- **Важная операционная ловушка (deploy_vps.py):** `--sync-attributes`/`--run-audit`, работая синхронно (не в фоне), дважды подряд обрывали SSH-сессию без единого байта вывода прямо на старте `npm run sync:attributes` (единственный тихий bulk-DELETE по ~1-2 млн строк без прогресса — похоже на обрыв по таймауту простоя NAT/firewall/sshd). Починено `transport.set_keepalive(30)` + периодические прогресс-логи в `sync-product-attributes.ts` (перед/после delete, каждые 20 батчей) — оба следующих прогона прошли чисто. Заодно починена кодировка вывода (`sys.stdout.reconfigure(encoding="utf-8")`) — кириллица в первом успешном аудите была нечитаемым мусором из-за консольной кодировки Windows.
- **Проверено на живом SKU:** «Хомут кабельный 4.8х300 DKC 25317SRT» (SKU 10646095, категория «Хомуты, стяжки кабельные») — раньше получал бы «Тип электротовара: Кабель» + «Количество жил: 300 жил», теперь фактов electrical/cable нет вообще.
- **Известные ограничения:** аудит-порог «cable_section ≥ 200мм² → verify manually» технически даёт 65 сработок на легитимных 240мм²-кабелях — это шум диагностики, не баг (можно поднять порог, не срочно).

### 2026-07-01 — Iter 71: фикс ложных electrical_product_type (камеры/кулеры/ТВ/консоли) ✅

- **Commits:** `1895482` (fix), `58fa2d4` (docs: known debt), `3a2799e` (feat: deploy_vps.py `--full-clean`/`--sync-attributes`) · **Backups:** `…source-backup-20260701130405.tar.gz` (первый деплой) → `…131332.tar.gz` (`--full-clean`, обошёл устаревший Turbopack-кэш) → `…132001.tar.gz` (`--sync-attributes`, пересчитал БД).
- **Контекст:** после Iter 70 (расширение парсера характеристик) на смоук-тесте обнаружено, что `extractElectricalProductType` в `product-attributes.ts` не гейтился по категории — слово-триггер («кабель», «box», «switch», «разъём») в описании интерфейса/упаковки ДРУГОГО товара давало ложный факт «Тип электротовара». Конкретные прод-примеры: камера видеонаблюдения с «...Кабель 1×RJ-45...» получала «Тип электротовара: Кабель» + «Количество жил: 1920/2560 жил» (число вытащено из разрешения «1920х1080»/«2560x1440» в названии); кулер ПК с «...Color Box»/«Retail Box» получал «Тип электротовара: Коробка/щит». Заодно найдены и починены ТВ/ноутбук с «разъемы 2x HDMI» → ложный «Коннектор» и Nintendo Switch → ложный «Выключатель».
- **Что вошло:**
  - `product-attributes.ts`: гейт по соседству с «N×интерфейс» (RJ-45/BNC/USB/HDMI/VGA/Type-C) для веток Кабель и Коннектор; исключение пакетных фраз «Color/Retail/Gift/OEM Box» для ветки Коробка/щит; исключение «Nintendo Switch» для ветки Выключатель; числовые границы `cable_cores ≤ 60`, `cable_section ≤ 300мм²`.
  - Опциональный `categoryName`-параметр прокинут через всю цепочку: `extractProductNameAttributes` → `extractProductNameSpecs` → `buildProductFacts` (карточка `/product/[slug]`) и → `buildProductAttributeRows` → `scripts/sync-product-attributes.ts` (batch-синк в БД). Категории «камеры»/«кулеры» полностью подавляют electrical-классификацию даже для голых модельных кодов без текстовых подсказок.
  - Новый read-only `scripts/audit-electrical-false-positives.ts` (`npm run audit:electrical`) — сканирует каталог на похожие коллизии.
  - Регрессионный корпус ложных срабатываний свёрнут в `it.each`-таблицу в `product-attributes.test.ts` — новую находку достаточно дописать одной строкой.
  - 277 → 286 тестов (vitest) + 8 python-тестов (`deploy_vps_test.py`, было 6) — все зелёные, `npm run lint`/`npm run build` чисто.
- **Важная ловушка, поймана и задокументирована в этой итерации:** обычный деплой (`rm -rf .next/cache`) НЕ подхватил правку в существующем файле `product-attributes.ts` — Turbopack persistent build cache отдавал старый скомпилированный вывод (уже описанная в CLAUDE.md ловушка #3, ранее виденная только на `route.ts`). Лечится добавленным флагом `python scripts/deploy_vps.py --full-clean` (полный `rm -rf .next`). **Ещё более важно:** даже после корректного передеплоя кода баг оставался виден на уже засинканных товарах — `getProductBySlug` (`catalog.ts`) сначала читает ПЕРСИСТЕНТНЫЕ строки `ProductAttribute` (source="name", записанные прошлым прогоном синка) и только потом дополняет их живым расчётом по названию; код-фикс сам по себе не переписывает уже сохранённые в БД строки. Лечится новым флагом `python scripts/deploy_vps.py --sync-attributes` (фоново перегоняет `npm run sync:attributes` на сервере, delete+reinsert всех `source="name"` строк по всему каталогу, ~250k товаров).
- **Проверено на живых прод-товарах после `--sync-attributes`:** камера Tiandy TC-C34QN (SKU 11261996, «...кабель,1xRJ-45», «2560x1440») — теперь только «Разрешение: 4 Мп», «Фокусное расстояние: 2.8 мм», «Тип камеры: Цилиндрическая», без «Тип электротовара»/«Количество жил»; кулеры PCCooler E90M (SKU 9890892, «...Retail Color Box») и Accord ACC-5P12PRO (SKU 10972895, «...Brown Box») — «Тип электротовара» пропал, остались «Размер вентилятора»/«Уровень шума»/«Скорость вращения»/«Подсветка ARGB/RGB». Базовые роуты (`/`, `/catalog`, `/cart`, `/favorites`, `/compare`, `/b2b`, `/gov`) — 200; `/account` — 307 на `/login` (ожидаемо, авторизация из Iter 69, не регрессия).
- **Известные ограничения:** `--sync-attributes` перечитывает ВСЕ активные товары и может занять несколько минут (фон, не блокирует деплой) — если сразу после деплоя проверять товар вручную, можно поймать промежуточное окно без name-based фактов. Аудит-скрипт (`audit:electrical`) написан, но полный прогон по всему каталогу с разбором находок ещё не делался — см. «Известные долги» в корневом `CLAUDE.md`.

### 2026-07-01 — Iter 70: характеристики на карточке товара — 5 категорий (ROADMAP #3) ✅

- **Commits:** `02491f2`..`47d8f26` (10 коммитов, по одной задаче на категорию/фичу) · **Backup:** `…source-backup-20260701091902.tar.gz`.
- **Контекст:** ROADMAP Tier 1 #3 — 39.5% продаваемых товаров (98 522 из 249 524) не имели ни одной технической характеристики, ещё 45% — только 1-2. Прод-замер (psql, read-only) показал, где парсер даёт наибольший разрыв при наибольшем объёме — выбраны 5 категорий: розетки/выключатели/рамки (14 915 шт.), встраиваемые варочные панели, роутеры/коммутаторы, камеры видеонаблюдения, кулеры ПК. Спека: `docs/superpowers/specs/2026-07-01-product-card-specs-coverage-design.md`, план: `docs/superpowers/plans/2026-07-01-product-card-specs-coverage.md`.
- **Bugfix:** гейт `extractCooktopAttributes` (`варочн\w*`) никогда не матчил «варочн**ая** панель» — `\w` не ловит кириллицу (тот же класс бага, что уже чинили для «духовой шкаф»). Один символ фикса закрыл большую часть разрыва варочных панелей.
- **Электрика (розетки/выключатели/рамки):** новые ключи `mount_type` (Открытая/Скрытая установка, только полные словоформы — без 2-буквенных ОП/СП во избежание коллизий), `gang_count` (кол-во клавиш/постов, цифрой и словом «двухклавишный»); `extractElectricalProductType` расширен (Рамка — только с числом постов/м, Переключатель — только с «проходной/клавиш», Терморегулятор); `extractColor` — Мокко/Шампань/Слоновая кость/Айвори/Графит.
- **Рефакторинг:** `ip_rating` дублировался в двух местах (camera + electrical) идентичным регексом — вынесен в общий `extractIpRating()`.
- **Новое семейство `network`** (роутеры/коммутаторы) — раньше категория вообще не матчилась ни в одно семейство (только universal-цвет), т.к. «Проводные» коллизировало с электрикой раньше в if-цепочке; добавлен явный маппинг ПЕРЕД electrical-веткой. Ключи: `port_count`, `poe_support`, `managed_type`.
- **Камеры:** `camera_lens_mm` теперь ловит голое число («3.6-3.6мм», «4мм») без слова «объектив/фокус» рядом (ограничено диапазоном 1-25мм, чтобы не спутать с габаритами корпуса); новые `camera_type` (Купольная/Цилиндрическая/Поворотная), `night_vision`.
- **Кулеры ПК:** новая гейтед-функция `extractCoolingAttributes` — `fan_size_mm`, `noise_db` (дБ/dBA), `rpm` (об/мин/RPM), `has_argb`.
- **Проверено на прод-цифрах (psql до/после):**

  | Категория | До | После |
  |---|---|---|
  | Розетки, выключатели и рамки | 48.9% | 31.5% |
  | Встраиваемые варочные панели | 86.8% | 47.6% |
  | Проводные роутеры/коммутаторы | 87.0% | 61.9% |
  | Камеры видеонаблюдения | 81.8% | 49.1% |
  | Кулеры/охлаждение ПК | 72.5% | 54.3% |

  Бэкфилл `npm run sync:attributes` на VPS: scanned=341216, written=391553. Смоук: по 1 товару из каждой категории на `/product/<slug>` — новые поля рендерятся; curl-смоук `/ /catalog /catalog/rozetki-vyklyuchateli-i-ramki /product/<slug> /cart /favorites /compare /b2b /gov` = 200, `/account` = 307 (ожидаемый редирект).
- **Проверки:** lint чисто, **275 тестов** (было 258, +17), build зелёный.
- **⚠️ Найден побочный шум (пред-существующий, НЕ мой код, вне скоупа):** `extractElectricalProductType` не гейтится по категории и срабатывает на ЛЮБОМ товаре, где в названии встречается «кабель»/«box»/«разъём»/«акб» как часть описания — у одной из тестовых камер (название содержит «Кабель 1×RJ-45») попутно вылезли «Тип электротовара: Кабель» + `cable_cores`/`cable_section` от разрешения «1920х1080»; у кулера — «Тип электротовара: Коробка/щит» от «Color Box» в конце названия. Это существовало ДО этой итерации (функция не менялась в этой части) — отдельный долг, не входил в спеку ROADMAP #3.
- **Осталось по ROADMAP #3:** ~66k товаров категорий-расходников (картриджи/чипы/дисплеи-запчасти) намеренно не в скоупе — их названия не содержат парсибельного текста (модель-код без описательных слов), нужен внешний источник данных, не расширение парсера.
...
- **Замер:** в установившемся режиме каталог ~1с, НО первый хит после деплоя (или истечения TTL) на крупной категории = ~10–14с (holodilniki 13с). Корень: data-кэш листингов `PRODUCT_QUERY_CACHE_SECONDS=600` (10 мин) истекал за 20 мин до 30-мин cron-прогрева (`warm_all.sh`) → окно холодного пере-расчёта; + деплой чистит `.next/cache` (всё холодное).
- **Фикс 1 (код):** `PRODUCT_QUERY_CACHE_SECONDS 600 → 3600` — под дизайн прогрева (`warm_all.sh`: «pairs with STOREFRONT_CACHE_SECONDS=3600»). Листинги остаются тёплыми весь цикл. Свежесть ок (каталог «под заказ» + админ-правки делают `revalidatePath("/catalog")`).
- **Фикс 2 (`deploy_vps.py`):** после pm2-рестарта греет кэш — `warm_cache.sh` (топ-12 синхронно) + `warm_all.sh` (полный хвост фоном, nohup). Деплой больше не оставляет холодное окно.
- **⚠️ Поймал и починил регрессию (CRLF):** деплой тарит РАБОЧУЮ копию; легаси `warm_*.sh` были `w/crlf` (чекаут до `.gitattributes eol=lf`) → bash на VPS давился `$'\r'`, сломав и мой пост-деплойный warm, И существующие cron-прогревы. Починка: `sed -i 's/\r$//' scripts/*.sh` на VPS + рабочие копии renormalized в LF (`rm` + `git checkout`, индекс уже LF) → будущие деплои везут LF. **ТРАП:** после правки/добавления `.sh` проверять `git ls-files --eol web-store/scripts/*.sh` (должно быть `w/lf`); иначе деплой ТИХО ломает скрипты (warm под `|| true`, деплой не падает).
- **Проверено:** после фикса holodilniki **13с → 1.15с**; топ-категории 1–3с. lint/test **258**/build зелёные.

### 2026-06-24 — Iter 67: фото-покрытие — честный фильтр «с фото» + восстановление ~19k фото от поставщика ✅

- **Commit (код):** `5448648` (фильтр) + `scripts/recover-gap-images.ts` (скрипт). **Восстановление фото — операционная правка данных на проде (не код-деплой).**
- **Контекст:** пункт #1 ROADMAP. Замер: **44% продаваемых (~86k из 195 869) без реального фото**.
- **Диагноз:** `Product.hasImage` = флаг поставщика `has_image` (`itp/products.ts:51`). 66 858 продаваемых с `false` — поставщик сам говорит «фото нет» (дотянуть неоткуда). **~19 200 с `true`, но 0 строк изображений** — синк `syncItpImages` гоняется из админки полным (все 210k `hasImage=true`) внутри веб-запроса и упирается в nginx-таймаут 180с, не доходя до хвоста SKU.
- **Фикс A (код `5448648`, задеплоен):** фильтр «только с фото» (`catalog.ts` `withPhoto`) → по РЕАЛЬНОМУ изображению `images.some(deleted=false)`, а не флагу. Проверено на телефоне: категория розетки baseline 1 заглушка → `?photo=1` **0 заглушек**.
- **Фикс B (операционно, `scripts/recover-gap-images.ts`):** read-only эксперимент подтвердил — поставщик ОТДАЁТ фото для gap (30/30 SKU, ~6.5 фото/товар). Точечный скрипт (та же Prisma-upsert + itpRpc, что в `syncItpImages`, но только gap-товары) прогнан фоном на VPS: **scanned 18 998, upserted 101 812, withImages 18 973, noImages 25, failed 0, ~16 мин**. Итог: gap **19 199 → 31**; продаваемых с реальным фото **109 820 → 128 988 (+19 168)**; «без фото» **44% → 34%**. Проверено end-to-end: восстановленное фото отдаётся `image/webp` по HTTP/2.
- **Запуск восстановления (повтор):** `cd /var/www/climat-simf.ru && node --env-file=.env --import tsx ./scripts/recover-gap-images.ts` (тест-слайс: `GAP_LIMIT=200 …`). На VPS node 20.20 + tsx 4.21.
- **Ловушки / системный долг:**
  - **Gap соберётся снова** для новых товаров (веб-синк фото всё ещё таймаутит). Системный фикс — гонять `recover-gap-images.ts` фоном по расписанию. **✅ Поставлен cron `30 6 * * *` (2026-06-25) после явного «ставь cron, подтверждаю». Лог `/var/log/img-gap-recover-cron.log`. Cron операционный (не в git) — на новом сервере повторить.**
  - Листинги кэшируются (`unstable_cache`, STOREFRONT_CACHE ~1ч) → новые фото на карточках появятся в течение ≤1ч; сам image-роут уже отдаёт WebP сразу.
- **Осталось по фото:** ~66k товаров без фото у поставщика — нужен внешний источник (бизнес-решение владельца).
