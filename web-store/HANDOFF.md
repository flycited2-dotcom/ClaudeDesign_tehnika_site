...
- **ОСТАЁТСЯ (действие владельца):** в `/admin/settings` вписать chat_id группы (отрицательный, −100…),
  сохранить → сделать тестовый заказ → проверить, что уведомление пришло в группу. Бот уже в группе.
