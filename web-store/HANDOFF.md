...
- **Ловушки / системный долг:**
  - **Gap соберётся снова** для новых товаров (веб-синк фото всё ещё таймаутит). Системный фикс — гонять `recover-gap-images.ts` фоном по расписанию (cron, напр. ежедневно после синка товаров). **Авто-установку cron заблокировал классификатор (Unauthorized Persistence) — нужно явное ОК владельца.**
