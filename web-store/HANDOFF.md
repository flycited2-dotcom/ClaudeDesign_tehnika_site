...
### 2026-08-30 — Вторая строка категорий на главной ✅

- **Commit:** `c52ac01`. **Production backup:** `/var/www/climat-simf.ru.source-backup-20260830010327.tar.gz`. **BUILD_ID:** `2oQnOudESIy5JhEoQcC0G`.
- На главную добавлена вторая desktop-строка из шести согласованных категорий: «Строительство и ремонт», «Товары для дома», «Товары для авто и мото», «Электроника», «Телевизоры», «Оборудование». Все карточки получили отдельные фотореалистичные PNG с прозрачным фоном; «Телевизоры» извлекаются по вложенному slug `televizory-9758` из полного дерева категорий.
- Проверено на production при viewport 1280x720: `.cat-grid` содержит ровно 12 карточек и шесть колонок, позиции 7–12, ссылки и пути изображений совпадают с согласованным порядком. `/` возвращает 200, PM2 online, B2B assistant active, stock monitor timer active и последний monitor Result=success.
- Локально пройдены lint, production build и 13 deploy-тестов. Все шесть новых изображений имеют размер 1122x1402 и alpha `0` во всех четырёх углах.
...
- VK Share pilot page deployed for the approved RUCELF SRW-12000-D card: `/share/vk/storefront-9758739` pins the approved `1254x1254` image through Open Graph instead of inheriting a random homepage product, uses a concise product title and expanded sales copy. Verification on VPS: targeted Vitest passed (`1` test), `npm run lint` passed in the isolated deployment copy, `npm run build` passed, PM2 `climat-simf-store` is `online`, `/` returns `200`, and the share page exposes `og:image=https://splithome.ru/static/cf-cards/storefront_9758739.png`. Source backup: `/var/www/climat-simf.ru.source-backup-20260823-104754.tar.gz`.
- VK Share comment fix deployed: `/share/vk/storefront-9758739/publish` now returns a `307` redirect to `vk.com/share.php` with both the product URL and the complete `comment` parameter (price, benefits, delivery, CTA and phone); the product page also exposes an `Открыть предпросмотр VK` button. Verification: targeted Vitest passed, lint and production build passed, `/` returns `200`, redirect location contains `Почему стоит выбрать` and `Рабочий диапазон 130–270 В`. Source backup: `/var/www/climat-simf.ru.source-backup-20260823-105724.tar.gz`.
- VK Share multiline comment fix deployed: product offer lines now use CRLF (`%0D%0A`) so every USP is passed to the VK textarea as a separate Windows-compatible line instead of a space-separated paragraph. Verification: targeted Vitest and lint passed in the isolated copy, production build passed, `/` returns `200`, and the redirect location contains CRLF between all eight `✅` items. Source backup: `/var/www/climat-simf.ru.source-backup-20260823-112520.tar.gz`.
- VK Share clipboard helper deployed after confirming that `vk.com/share.php` normalizes both LF and CRLF query-string whitespace. `/share/vk/storefront-9758739/publish` now renders the formatted multiline offer, copies it through a user-clicked `Скопировать УТП и открыть VK` button, and opens a clean VK share form for `Ctrl+V`; this preserves every line break without cross-origin automation. Verification: targeted Vitest, lint and isolated build passed; production build passed; `/` returns `200`; helper page returns `200` and contains the copy/open CTA. Source backup: `/var/www/climat-simf.ru.source-backup-20260823-112957.tar.gz`.
- Public/internal VK page separation deployed after end-to-end recipient testing: the customer-facing `/share/vk/storefront-9758739` no longer exposes `Открыть предпросмотр VK` or `Скопировать УТП`; the public page now contains only product information and sales CTAs. The temporary public `/publish` helper and its client component were removed, so `/share/vk/storefront-9758739/publish` returns `404`. Verification: targeted Vitest, lint and isolated build passed; production build passed; `/` returns `200`; production HTML contains no internal VK UI. Source backup: `/var/www/climat-simf.ru.source-backup-20260823-114041.tar.gz`.
- Three customer-safe VK post pages deployed: `/share/vk/shtil-is3000rt`, `/share/vk/powerman-avs-2000s`, and `/share/vk/powerman-back-pro-1050`. Each returns `200`, exposes its own `1254x1254` Open Graph image, and contains no internal VK controls or delivery claim in Open Graph. The remaining delivery wording was also removed from the RUCELF share product. Prisma warned that `db push` would drop three populated tables; data loss was rejected and the UI-only deploy completed without a schema change. Stale remote-only `src/app/stock-order/[sku]/route.ts`, `src/lib/b2b-assistant-offer-store.ts`, and `scripts/b2b-assistant-bot.ts` were removed by clean `src`/`scripts` synchronization; they remain recoverable from `/var/www/climat-simf.ru.source-backup-20260823205426.tar.gz`. PM2 `climat-simf-store` is `online`.
- VK community token created three postponed text posts because neither the community token nor the refreshed VK ID token has native photo-upload permission: post `3` (ШТИЛЬ, `2026-08-23 23:09 +03:00`), post `4` (POWERMAN AVS 2000S, `2026-08-24 17:00 +03:00`), post `5` (POWERMAN Back Pro 1050, `2026-08-25 10:00 +03:00`). All texts and public links are present and contain no delivery claims. Attach the matching PNG manually in the postponed-post editor before each publication; the group token can call `wall.post` but VK rejects `wall.edit`, `wall.delete`, `photos.getWallUploadServer`, and URL previews for this token type.
